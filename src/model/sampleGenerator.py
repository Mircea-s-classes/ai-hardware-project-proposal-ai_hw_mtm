...
for audio, label in train_data:
    if label == yes_label_index:
        ...
        # Pad and quantize features as you did before
        features_padded = np.pad(features, (0, 490 - features.shape[0]), mode='constant')
        max_abs_value = np.max(np.abs(features_padded))
        quantization_factor = 127 / max_abs_value
        quantized_features = np.round(features_padded * quantization_factor).astype(np.int8)
        quantized_features_clipped = np.clip(quantized_features, -128, 127)

        # Print the quantized 490-element vector
        print(", ".join(map(str, quantized_features_clipped)))
